...
class PixelArtApp:
    def __init__(self, root):
        ...
        self.labeled_output_path = "output_labeled.png"
    ...
    def setup_ui(self):
        ...
        btn_labeled = tk.Button(frame_top, text="🎯 Сетка и подписи", command=self.generate_labeled_pixel_art)
        btn_labeled.grid(row=0, column=11, padx=5)

        btn_save_labeled = tk.Button(frame_top, text="💾 Сохранить с подписями", command=self.save_labeled_output)
        btn_save_labeled.grid(row=0, column=12, padx=5)
    ...
    def generate_pixel_art(self, grid_size, n_colors=16):
        ...
        self.color_hex_map = {
            tuple(color): (f'#{color[0]:02X}{color[1]:02X}{color[2]:02X}', abbr)
            for color, abbr in zip(clustered_colors, abbreviations)
        }
        ...
        for y in range(grid_size[1]):
            for x in range(grid_size[0]):
                ...
                ax.add_patch(patches.Rectangle(
                    (x, grid_size[1] - y - 1), 1, 1, linewidth=0,
        ...
        for i, color in enumerate(clustered_colors):
            ...
            ax.add_patch(patches.Rectangle((grid_size[0] + 1, y_pos), 2, 1.5,
                                           edgecolor="black", facecolor=np.array(color)/255))
            ax.text(grid_size[0] + 3.4, y_pos + 0.75, f"{hex_color} ({label})", fontsize=12, va='center')
        ...
        for y in range(grid_h):
            for x in range(grid_w):
                ...
                ax.add_patch(patches.Rectangle(
                    (x, grid_h - y - 1), 1, 1,
                    edgecolor='gray',
                    facecolor=(brightness, brightness, brightness, 0.3)
                ))
                ax.text(
                    x + 0.5, grid_h - y - 0.5,
                    label,
    ...
    def generate_labeled_pixel_art(self):
        if not hasattr(self, 'clustered_img'):
            messagebox.showerror("Ошибка", "Сначала сгенерируйте пиксель-арт.")
            return
        ...
        for y in range(grid_h):
            for x in range(grid_w):
                color = self.clustered_img[y, x]
                _, label = self.color_hex_map[tuple(color)]
                ax.add_patch(patches.Rectangle((x, grid_h - y - 1), 1, 1, linewidth=0.5, edgecolor='gray', facecolor=np.array(color)/255))
                ax.text(x + 0.5, grid_h - y - 0.5, label, fontsize=10, ha='center', va='center', color='black')

        plt.tight_layout()
        plt.savefig(self.labeled_output_path, dpi=150)
        plt.close()

        self.show_preview(self.labeled_output_path)
    ...
    def save_labeled_output(self):
        path = filedialog.asksaveasfilename(defaultextension=".png", filetypes=[("PNG files", "*.png")])
        if path:
            try:
                Image.open(self.labeled_output_path).save(path)
                messagebox.showinfo("Готово", f"Изображение с подписями сохранено:\n{path}")
            except Exception as e:
